...
import {
    AlertCircle, ArrowRight, Award, Calendar, CheckCircle2, ChevronDown, ChevronUp, Phone, Target, Zap,
} from "lucide-react"
...
const RECOMMENDATION_COPY: Record<string, {
    why: string
    revenue: string
}> = {
    "render-blocking-resources": {
        why: "Render-blocking CSS/JS delays First Contentful Paint and Largest Contentful Paint. When the browser must download, parse, and execute these assets before painting, users stare at a blank screen and bounce—especially on mobile or slow networks.",
        revenue: "Cutting render-blocking time routinely improves time-to-interactive and can lift mobile conversion rates by ~10–30% via reduced abandonment and higher engagement with above-the-fold CTAs.",
    }, "unused-javascript": {
        why: "Excess JavaScript inflates download, parse, and execution cost, taxing CPU and memory on mid-range phones. This degrades interactivity (INP) and causes input delay and jank.",
        revenue: "Trimming unused JS reduces TTI and interaction latency, which correlates with more form completions and checkout starts. Expect ~5–20% lift in task completion for JS-heavy pages.",
    }, "unused-css-rules": {
        why: "Bloated CSS blocks rendering and increases style recalculation costs. Users wait longer to see a stable layout, increasing the chance of bounce before any value is communicated.",
        revenue: "Purged CSS speeds first render and reduces layout shifts, keeping users focused on your offer. This commonly improves micro-conversion events (menu opens, scroll depth) by ~5–15%.",
    }, "uses-responsive-images": {
        why: "Shipping desktop-sized images to mobile devices wastes bandwidth and time. Without srcset/sizes, small screens download large assets they don’t need.",
        revenue: "Right-sized images reduce page weight dramatically, improving perceived speed and product viewing. Sites often see ~10–25% more gallery interactions and add-to-cart attempts.",
    }, "uses-optimized-images": {
        why: "Uncompressed PNG/JPEG—and especially animated GIF—can dominate transfer size. Modern formats (WebP/AVIF) preserve quality at a fraction of the bytes.",
        revenue: "Optimized media accelerates hero and product imagery, lifting engagement and decreasing bounce on landing pages. Typical gains: ~10–20% better session duration and ~5–15% conversion lift.",
    }, "uses-text-compression": {
        why: "Uncompressed HTML/CSS/JS costs extra round trips and longer downloads. Enabling gzip/brotli/zstd slashes transfer size without code changes.",
        revenue: "Lighter text assets boost first render and interactivity, improving form start rates and search landing retention. Expect ~5–12% improvements on pages with large script bundles.",
    }, "efficient-animated-content": {
        why: "Large GIFs and unthrottled animations are CPU-intensive and heavy to download. Video with proper codecs or CSS transforms are far more efficient and battery-friendly.",
        revenue: "Replacing heavy animations reduces stutter, especially on mobile, improving perceived polish and trust—often translating to ~5–10% more CTA clicks on animated hero sections.",
    }, "server-response-time": {
        why: "High TTFB delays everything: HTML, critical CSS/JS discovery, and data fetching. Slow origin time compounds with network latency, harming all downstream metrics.",
        revenue: "Reducing TTFB via caching, DB/index tuning, and edge deployment improves all funnel steps. Many sites see ~15–35% bounce reduction on paid traffic and notable ROAS gains.",
    }, "total-byte-weight": {
        why: "Every extra megabyte extends load on constrained networks and older devices. Heavy pages are especially punishing for first-time visitors without a warm cache.",
        revenue: "Lowering total bytes expands your effective audience and increases successful landings from ads/social. Expect ~8–20% more users reaching key content and CTAs.",
    }, "image-alt": {
        why: "Missing alt text breaks screen-reader comprehension and harms image SEO. Descriptive alt also provides context if the image fails to load.",
        revenue: "Better accessibility widens reach and lowers friction for assistive tech users. It can also surface more image search traffic, contributing incremental qualified visits.",
    }, "color-contrast": {
        why: "Low contrast text is hard to read on mobile and in sunlight, increasing cognitive load and abandonment—especially for older users and those with low vision.",
        revenue: "Improved legibility boosts time-on-page and reduces form errors, often yielding ~3–8% more successful submissions on contact/quote flows.",
    }, "heading-order": {
        why: "Disordered headings confuse assistive technologies and harm content scanning for all users. A single, descriptive H1 with logical H2/H3 helps both UX and SEO.",
        revenue: "Cleaner information architecture improves comprehension and SERP snippet mapping, aiding click-through and on-page engagement—which supports downstream conversions.",
    }, "html-has-lang": {
        why: "The lang attribute enables correct pronunciation, hyphenation, and dictionary rules for screen readers and search engines.",
        revenue: "Proper language tagging improves accessibility compliance and can enhance international SEO targeting, adding incremental qualified traffic.",
    }, "aria-allowed-attr": {
        why: "Invalid ARIA undermines assistive tech, making controls unusable or misleading. This erodes trust and blocks key actions for some users.",
        revenue: "Fixing ARIA improves completion of navigation and forms, recovering conversions otherwise lost to inaccessible UI—typically a measurable uplift on lead gen forms.",
    }, "link-name": {
        why: "Links without accessible names (“Read more”, icon-only) lack context for screen readers and keyboard users, reducing discoverability of critical paths.",
        revenue: "Clear, descriptive link names improve click-through to money pages (pricing, booking, product), increasing funnel entry rates by ~5–12%.",
    }, "no-vulnerable-libraries": {
        why: "Known CVEs in client libraries expose users to XSS and supply-chain risks. Security warnings in Chrome/Edge also damage credibility.",
        revenue: "Removing warnings boosts trust and completion of sensitive steps (checkout, payment, login). Security-related friction often suppresses conversion by several points until fixed.",
    }, "deprecations": {
        why: "Deprecated APIs can break unexpectedly with browser updates, causing regressions in critical flows with no code changes from your team.",
        revenue: "Proactive migration prevents outages that tank revenue. Teams that stay current avoid sudden conversion drops tied to silent browser changes.",
    }, "password-inputs-can-be-pasted": {
        why: "Blocking paste increases login friction, penalizes password manager users, and raises reset requests—hurtful on mobile.",
        revenue: "Allowing paste raises successful login and checkout completion rates, commonly improving return-user conversion by ~3–7%.",
    }, "meta-description": {
        why: "Missing or generic meta descriptions lead to poor SERP snippets. Compelling summaries drive higher click-through for the same rank.",
        revenue: "Higher CTR increases traffic without additional ad spend. Even +0.5–2.0 pp CTR on high-intent queries can yield meaningful lead/revenue gains.",
    }, "document-title": {
        why: "A weak or duplicated title confuses users and search engines about page purpose. Titles are the primary SERP headline.",
        revenue: "Sharper titles lift CTR and brand recall, compounding SEO value over time and feeding more qualified sessions into your funnel.",
    }, "hreflang": {
        why: "Incorrect/missing hreflang causes wrong-locale pages to rank, raising bounce and decreasing relevance for international users.",
        revenue: "Proper locale targeting recovers qualified traffic and improves local conversion rates in served languages/regions.",
    }, "canonical": {
        why: "Absent or conflicting canonicals dilute link equity across duplicates and can cause the wrong page to rank.",
        revenue: "Consolidating signals improves ranking stability for revenue pages, capturing more consistent organic traffic.",
    }, "viewport": {
        why: "Without a responsive viewport, mobile users see zoomed-out, tiny text and tap targets—immediate abandonment risk.",
        revenue: "Mobile-friendly rendering increases engagement on small screens, typically improving mobile conversion by ~10–25% depending on baseline.",
    }, "robots-txt": {
        why: "A misconfigured robots.txt can block critical pages or assets from being crawled, suppressing rankings and rich results.",
        revenue: "Fixing crawlability restores organic visibility and prevents traffic loss from accidental disallows—often recovering high-intent visits.",
    },
}

// Build a reverse lookup from short description -> rule id
const RULE_ID_BY_DESCRIPTION: Record<string, string> = Object.fromEntries(Object.entries(RECOMMENDATION_MAP).map(([id, desc]) => [desc, id]))
...
function RecommendationCard({rec, index}: { rec: string; index: number }) {
    ...
    const impactColors: Record<string, string> = {
        high: "border-red-200 bg-red-50", medium: "border-amber-200 bg-amber-50", low: "border-blue-200 bg-blue-50",
    }
